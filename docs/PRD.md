...
> 个人职业门户站项目需求文档。
> 仓库定位：独立长期项目，与 ScholarMind / ScriptLens 等作品集项目平级。
...
| 与 ScholarMind / ScriptLens 关系 | 展示 Demo、视频、截图、源码、架构文档与复盘文章；不嵌入其业务代码 |

成功状态：

- 已投递的 PDF 简历即使不再更新，HR / 面试官仍可通过固定二维码或短链访问最新在线简历。
- 访问者 30 秒内能判断候选人的方向、学历、求职状态、代表项目、技术栈和联系方式。
- 技术面试官 3 分钟内能看到项目证据链：职责、架构、难点、Demo、代码、文档和复盘。
...
| 技术面试官 | 已知候选人有项目，想快速判断技术深度 | 能看到架构、职责、代码、Demo、复盘和可追问点 | `/projects` → `/projects/<slug>` → GitHub / Demo |
...
| 10 分钟 | 完整在线简历、项目架构、Demo 视频、GitHub 仓库、技术文章 |
...
| `broken` | 外链失效或 Demo 不可用 | 显示不可用标记，不让访问者误点 |
...
| 项目阶段完成 | 每个里程碑后 | 截图、Demo、架构图、职责、复盘、可追问点 |
...
- ScholarMind / ScriptLens 等项目的演示视频、截图、架构图、源码链接与复盘接入本站。
...
| P1 | Proof of Work 区域 | 聚合 Demo、截图、架构图、关键文档和文章 |
...
- 不做在线表单后端；联系入口用邮箱、GitHub、微信二维码、飞书主页。
- 不在站点上展示真实手机号。
- 不在站点上展示薪资期望。
...
/projects/scholarmind     ScholarMind 详情：职责、架构、Demo、视频、GitHub、复盘
/projects/scriptlens      ScriptLens 详情
/articles                 技术文章索引：飞书外链、摘要、标签、发布时间
/resume                   在线简历：最新履历，可打印为 PDF
/contact                  联系方式聚合：邮箱、GitHub、微信二维码、飞书主页
```
...
| 主体 | 单列（移动端）/ 最多两列（桌面端） |
| 证据入口 | 代表项目、Demo、GitHub、文章、简历至少出现一个明确入口 |
| 页脚 | 邮箱、GitHub、微信二维码入口、飞书主页、最后更新时间 |
...
| P0 | 展示 3 个代表项目卡片，首批至少包含 ScholarMind 和 ScriptLens |
...
| P1 | 展示 Proof of Work 条带：Demo、GitHub、文章、架构图、视频 |
...
| P0 | 每张卡片提供详情页入口 |
| P1 | 标识项目成熟度：已上线 / Demo 可用 / 开发中 / 归档 |
| P1 | 标识证据类型：Demo、视频、GitHub、架构图、复盘文章 |
...
|---|---|
| P0 | 邮箱一键复制 |
| P0 | GitHub、飞书主页、技术博客主页外链 |
| P0 | 微信二维码弹层展示，不明文展示微信号和手机号 |
| P1 | 展示可联系时间和偏好方式 |
| P1 | 提供邮件主题建议，如“AI 应用工程岗位沟通” |
...
| 作品集 | 至少 1 个完整作品集详情页（ScholarMind），包含职责、架构、难点、Demo、GitHub、复盘 |
| 证据链 | featured 项目至少包含 Demo / 截图 / 视频 / GitHub / 架构图中的两类证据 |
| 移动端 | iPhone SE（375px 宽）和常见安卓设备无横向滚动条、无字号过小 |
| 桌面端 | 1280px / 1440px / 1920px 三档下排版稳定 |
| 性能 | 首页 Lighthouse Performance >= 90 |
| SEO | 每页有独立 title / description / og:image |
| 联系 | 邮箱可复制，GitHub 可访问，微信二维码可识别 |
...
| M3 作品集主体 | ScholarMind 详情页、ScriptLens 卡片、项目索引、证据链组件 | 2 天 |
